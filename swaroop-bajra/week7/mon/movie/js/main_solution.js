$(document).ready(function(){
  ...
  $('#searchForm').on('submit', function(e){
    ...
    $('#movieDetails').hide();
    $('#searchResults').html('<p>Loading..</p>').show();
...
const performMovieSearch = function( queryText ){
  ...
  xhr.onload = function(){
    ...
    for (let i = 0; i < data.results.length; i++) {
      ...
      $('#searchResults').append(`<img movieID="${movie.id}" class="result" src="${ imgURL }">`);

    } // for

    // Moved to the document ready
    // Attach a single click handler function to all of the result images
    // $('img.result').on('click', function(){
    //   const movieID = $(this).attr('movieID');
    //   // console.log('Image Clicked', $(this).attr('movieID'));
    //   showMovieDetails(movieID);
    // }); // img.result click handler
