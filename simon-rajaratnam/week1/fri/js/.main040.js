...
// Attempting to try and step it up.. What will happen if I make stations objects that have a name, maybe stop number and connecting line (which could be an array of lines that it connects across). Then if the trip required changing lines, each station would return lines it could cross. None for most but at junctions, could list the lines.

// const theGrid = stations
//   line : N,
//   stopNumber : 1,
//   stationName :
//   connectLines : [],
...
const theGrid = {
  'N' : [
    {name : 'Time Square',
    findDestination : function(destinationLine, destinationStation)
    }
  ]
}
...
const singleLineTrip = function( line, start, end) {
    ...
    const route = [];   // holds the travel plan.
...
const tripPlan = function( startLine, startStation, endLine, endStation ) {
    ...
    if ( startLine === endLine ) {    // if the startStation and endStation are on the same line.

      const singleLineRoute = singleLineTrip( startLine, startStation,endStation);
      ...
      const singleLineRoute = singleLineTrip( startLine, startStation, 'Union Square');
      const multiLineRoute = singleLineTrip( endLine, 'Union Square', endStation);
