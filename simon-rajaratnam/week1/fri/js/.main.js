...
//     //  console.log('entered backwards stops');
//       numberOfStops = numberOfStops * -1    // change it to a positive number.
//
//       for ( i=startIndex; i >= endIndex; i-- ) {
//         route[i] = line[i-1];   // i-1 because we don't include the starting station.
//         route.reverse();
//       }
...
//       }
//     } // end if
//       console.log(`You must travel through the following stops on the N line: ${route.toString()}.`)
...
const singleLineTrip = function( line, start, end) {
    route.length = 0;   // resets the array for a new trip.
    ...
    if ( numberOfStops < 0 ) {    //travel backwards
      ...
      for ( i=0; i < numberOfStops; i++ ) {
        route[ i ] = line[ (startIndex - i)-1 ];   // -1 because we don't include the starting station.
      }
    } else {    // travel forwards
      //console.log('entered forwards stops');
      for ( i=0; i < numberOfStops; i++ ) {
        route[ i ] = line[ (startIndex + i)+1 ];   // +1 because we don't include the starting station when reporting.
      }
    } // end if
  ...
  singleLineTrip(N,'Times Square','Union Square');
  singleLineTrip(N,'Union Square','Times Square');
