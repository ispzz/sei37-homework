...
//
// // createParagraph() shows output similar to this:
...
const createParagraph = function(input) {
  let tag = document.createElement("p");
  let text = document.createTextNode(input);
  tag.appendChild(text);
  let element = document.getElementById("outPutDirections");
...
const output = {
  intro: function(lineA, lineAArray, stationAIndex, lineB, lineBArray, stationBIndex) {
    ...
    createParagraph(`You have entered that you are currently at ${lineAArray[stationAIndex]} station on the ${lineA} line and would like directions to ${lineBArray[stationBIndex]} station on the ${lineB} line.`);
    createParagraph(`Please follow the below steps for the shortest possible route:`);
  },//intro function end
  travelSteps: function(destination, lineDestination, passedStationsArray, stepCounter) {
    createParagraph(`${stepCounter}. Board the train moving toward ${lineDestination} station.`);
    stepCounter++;
    ...
    if(passedStationsArray.length > 0){
      ...
      if(passedStationsArray.length > 1) {
        ...
        createParagraph(`${stepCounter}. Wait until you have passed stations ${passedStations}.`);
      } else {
        createParagraph(`${stepCounter}. Wait until you have passed ${passedStationsArray} station.`);
      }
      stepCounter++;
    }
    createParagraph(`You have now have arrived at ${destination} station.`);
    return stepCounter;
  },//travelSteps function end
  //Output to switch train lines
  switchLines: function(lineB, stepCounter) {
    createParagraph(`${stepCounter}. Exit the train and make your way to Line ${lineB}.`);
    stepCounter++;
    return stepCounter;
  },
  moveWithinStation: function(lineB, stopOver, stepCounter){
    createParagraph(`${stepCounter}. Make your way to line ${lineB} within ${stopOver} station.`);
    stepCounter++;
    return stepCounter;
  },
...
const populateList = function(selection, selectID, labelID) {
  const line = selection.value;
  const lineArray = mtaLines[line];
  let stationListLabel = document.getElementById(labelID);
  let stationList = document.getElementById(selectID);
  stationList.setAttribute('style', 'display:block');
  stationListLabel.setAttribute('style', 'display:block');
  let length = stationList.options.length;
  if(length > 0){
    for (i = length-1; i >= 0; i--) {
      stationList.options[i] = null;
    }
  }
  for (let i = 0; i < lineArray.length; i++) {
    let option = document.createElement("option");
    option.text = lineArray[i];
    stationList.add(option);
  }
  //return the chosen train line
  return line;
}


  const submitForm = function() {
    document.getElementById("outPutDirections").innerHTML = null;
    let lineA = document.getElementById("startingLine");
    lineA = lineA.value;
    let stationA = document.getElementById("startingStation");
    stationA = stationA.value;
    let lineB =document.getElementById("endingLine");
    lineB = lineB.value;
    let stationB = document.getElementById("endingStation");
    stationB = stationB.value;
    planTrip(lineA, stationA, lineB, stationB);
    document.getElementById("outPutDirections").setAttribute('style', 'display:block');
}
