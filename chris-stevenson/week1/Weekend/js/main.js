...
//Variable to store trainDirection and  lineDestination
let lineDestination;
//Array to hold stations we've passed
let passedStationsArray = [];
//Step counter for each instruction given.
let stepCounter = 1;
...
const travel = function(start, end, array) {
  //if starting station comes before end station
  if(start < end) {
    //Loop through future stations
    for (var i = start; i < array.length; i++) {
      //If we arrive at end
      if(array[i] === array[end]){
        lineDestination = array[array.length - 1];
        break;
      }
      //For each station we pass but do not stop at, add to the passedStationsArray array
      passedStationsArray.push(array[i]);
    }//for loop moving forward
  } else { //else we must travel in opposite direction
    for (var i = start; i > -1; i--) {
      //If we arrive at end
      if(array[i] === array[end]){
        lineDestination = array[0];
        break;
      }
      //For each station we pass but do not stop at, add to the passedStationsArray array
      passedStationsArray.push(array[i]);
    }//for loop moving backwards
  }//else
...
const output = {
  ...
  travelSteps: function(destination) {
...
const planTrip = function(lineA, stationA, lineB, stationB){
  ...
  if(lineA === lineB){
    travel(stationAIndex, stationBIndex, lineAArray);
    output.travelSteps(stationB);
  } else {
    //Travel to the shared station in the network
    travel(stationAIndex, lineAArray.indexOf(stopOver), lineAArray);
    output.travelSteps(stopOver);
    //Exit function if you have arrived at the shared station.
    if(stationB === stopOver){
      return;
    }
    //Switch to different train line
    console.log(`${stepCounter}. Exit the train and make your way to line ${lineB}.`);
    stepCounter++;
    travel(lineBArray.indexOf(stopOver), stationBIndex, lineBArray);
