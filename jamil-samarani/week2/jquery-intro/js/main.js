...
//Calling .attr() with ONE argument means GET the current value of the specified attribute, i.e. 'src'
const imgSrc = $billImg.attr('src');
console.log( imgSrc );

//Calling .attr() with TWO arguments means SET the attribute (the first argument) to be this new value (the second argument)
$billImg.attr("src", "http://www.placekitten.com/400/300");

//Let's change the contents of a tag - in this case, the <li id="bestDog">
const $bestDogListItem = $( "#bestDog");
//console.log( $bestDogListItem);

//Calling .html() with ONE argument means SET the innerHTML to be that argument
$bestDogListItem.html("Nookie (as if...yeah right...nice try, Mikaela)");

// Implicit iteration: every tag that matches the query will have its HTML contents changed!
$("p").html("HAXXXXXOOORREEEDDD!!!1!");
